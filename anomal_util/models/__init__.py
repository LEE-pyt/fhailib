...
from anomal_util.models.fastflow import Fastflow
from anomal_util.models.patchcore import Patchcore
...
def get_model(config: DictConfig | ListConfig) -> AnomalyModule:
    ...
    if config.model.name in get_available_models():
        module = import_module(f"anomal_util.models.{config.model.name}")
